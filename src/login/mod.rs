...
pub async fn login(driver: &WebDriver) -> WebDriverResult<Value> {
	...
	actions::click_from(driver, &By::Tag("a"), 1, 10).await?;

	actions::send_keys(driver, &By::Tag("input"), data["email"].as_str().unwrap()).await?;
	println!("{}", "Email entered".blue());

	actions::click_from(driver, &By::Tag("button"),1,  10).await?;
	println!("{}", "Continuing...".blue());

	sleep(Duration::from_secs_f64(0.5)).await;

	actions::send_keys(driver, &By::Tag("input"), data["password"].as_str().unwrap()).await?;
	println!("{}", "Password entered".blue());

	actions::click(driver, &By::Tag("button")).await?;
...
pub async fn start_assignment(driver: &WebDriver) -> WebDriverResult<()> {
	...
	actions::click(driver, &By::Css(".btn-pushable")).await?;
	println!("{}", "Assignment started\nLoading assignment...".blue());
	actions::click(driver, &By::Css("div[class='sc-hdWpuu cCeQmZ']")).await?;
